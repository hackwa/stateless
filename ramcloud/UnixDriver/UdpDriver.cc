    ...
    , readerThreadExit(false)
{
    ...
    int fd = sys->socket(AF_UNIX, SOCK_DGRAM, 0);
    if (fd == -1) {
	...
	strncpy(addr.sun_path, locatorString.c_str(), sizeof(addr.sun_path)-1);
	int r = sys->bind(fd, (struct sockaddr*)&addr, sizeof(addr));
        if (r == -1) {
            int e = errno;
            sys->close(fd);
	...
	strncpy(address.sun_path, "unixclient", sizeof(address.sun_path)-1);
	int r = sys->bind(fd, (struct sockaddr*)&address, sizeof(address));
        ...
        LOG(NOTICE, "UdpDriver using port %s", "unixsocket");
    }
                      ...
                      int priority)
{
    ...
    const sockaddr_un* a = &(static_cast<const UnixAddress*>(addr)->address);
   
    struct sockaddr_un u;
    memset(&u, 0, sizeof(u));
    u.sun_family = AF_UNIX;
    strncpy(u.sun_path, a->sun_path, sizeof(a->sun_path)-1);
